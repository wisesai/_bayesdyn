...
#  		ff 		-	(Tx1) filtered estimated fits (actual data up to t=p) 
#  		fs 		-	(Tx1) snmoothed estimated fits (actual data up to t=p) 
#
...
ff <- matrix(NA,nrow=T,ncol=1)
fs <- matrix(NA,nrow=T,ncol=1)
ff[1:p] <- arx[1:p]
fs[1:p] <- arx[1:p]
